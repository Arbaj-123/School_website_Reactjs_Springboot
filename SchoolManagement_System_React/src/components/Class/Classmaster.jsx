...
const Classmaster = () => {
  ...
  const delData = (id) => {
    axios
      .delete(`http://localhost:8080/class_del/` + id)
      .then(() => {
  ...
  const filteredTable = () => {
    const filteredData = bio.filter(
      (vall, ind) =>
        vall.cls_name.toLowerCase().substring(0, searchText.length) ===
          searchText ||
        vall.cls_id.toLowerCase().substring(0, searchText.length) === searchText
    );
    if (searchText !== "") {
      setBio(filteredData);
    } else {
      ...
      <div className="container mt-5">
        <div className="row">
          <div className="col-md-12">
            ...
            >
              <NavLink className="btn btn-primary" to="/addClass">
                Add New
              </NavLink>
            </div>
          </div>
            ...
            return (
              <>
                <tr>
                  ...
                  <td>{val.cls_name}</td>
                  <td>
                    <NavLink className="btn btn-success" to={`/edit/${val.id}`}>
                      <FaEdit />
                    </NavLink>
                  </td>
                  <td>
